import React, { useRef, useEffect, forwardRef, useImperativeHandle, useState, useCallback } from 'react'; // Added useCallback import
import '../Video.css';
...
const Video = forwardRef(({ embedId, stopTime, startTime = 0, onPlayerInitialized }, ref) => {
  ...
  // Ref to store the latest stopTime value
  ...
  const onPlayerReady = useCallback((event) => { // Wrapped in useCallback
  ...
  }, [onPlayerInitialized]); // Dependency: onPlayerInitialized

  // Callback function when the player's state changes
  const onPlayerStateChange = useCallback((event) => { // Wrapped in useCallback
  ...
  }, [intervalRef, playerRef, stopTimeRef]); // Dependencies: intervalRef, playerRef, stopTimeRef
  ...
  }, [onPlayerReady, onPlayerStateChange]); // Update refs when callbacks change
